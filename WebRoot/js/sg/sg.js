...
function yzLogin(){
	...
	"		<a href='javascript:void(0);' id='closeLogin' onclick='closepanel()'>X</a>"+
	"		<div class='loginbox'>"+
	"			<div class='login-tab'>"+
	"				<ul>"+
	"					<li class='curent'>邮箱登录 </li>"+
	"					<li>手机登录 </li>"+
	"				</ul>"+
	"			</div>"+
	"			<div class='login-inps'>"+
	"			<!-- 邮箱登录 -->"+
	"			<div class='login-inp'>"+
	"				<label>"+
	"					<i>邮箱地址：</i>"+
	"					<input type='text' name='emailname' id='email-inp' placeholder='注册的邮箱全称' autofocus='autofocus'/>"+
	"				</label>  "+
	"				<label>"+
	"					<i>登录密码：</i>"+
	"					<input type='password' name='emalpassword' id='email-pwd' placeholder='登录密码' /> "+
	"				</label>"+
	"				<label>"+
	"					<input type='button'  onclick='emailLogin()' id='emailSubmit' value='登 录' /> "+
	"				</label>"+
	"				<label  class='day7'>"+
	"					<i></i>"+
	"					<!--<input type='checkbox'  value='7' id='email-week'/> <span class='autologin'>一周自动登录</span>--> <span class='autologin' id='forgetPwd'>忘记密码？</span>"+
	"				</label>"+
	"			</div>"+
	"			<!-- 手机登录 -->"+
	"			<div class='login-inp'>"+
	"				<label>"+
	"					<i>手机号码：</i>"+
	"					<input type='text'  id='phone-inp' placeholder='注册的手机号码' autofocus='autofocus'/>"+
	"				</label>  "+
	"				<label>"+
	"					<i>登录密码：</i>"+
	"					<input type='password'  id='phone-pwd'  placeholder='登录密码' /> "+
	"				</label>"+
	"				<label>"+
	"					<input type='button' onclick='phoneLogin()' value='登 录' /> "+
	"				</label>"+
	"				<label  class='day7'>"+
	"					<i></i>"+
	"					<!--<input type='checkbox'  value='7' id='phone-week'/> <span class='autologin'>一周自动登录</span>--> <span class='autologin'>忘记密码？</span>"+
	"				</label>"+
	"			</div>"+
	"		</div>"+
	"	</div>"+
	...
	/* 登录方式选择 */
	$(".login-tab").find("li").on("click",function(){
		var index = $(this).index();
		$(".login-inp").eq(index).show().siblings().hide();
		$(".login-tab").find("li").eq(index).addClass("curent").siblings().removeClass("curent");
	});
...
function emailLogin(){
	...
	var status = $("#email-week").attr("checked");
	var iskeep;
	if ($("#email-week").is(":checked")){
		iskeep=$("#email-week").val();
	}
	if(!is_email(email)){
		messageBox("请输入正常邮箱地址!");
	}
	if(isEmpty(pwd)){
		messageBox("密码不能为空！");
	}
	$.ajax({
		...
		data:{
			"email":email,
			"pwd":$.yzpwd.encode(pwd),
			"iskeep":iskeep
		},
		beforeSend:function(){
			$("#emailSubmit").removeAttr("onclick");
		},
		success:function(response){
			$("#emailSubmit").attr("onclick","emailLogin()");
			var msg = response.result.msg;
			if(isNotEmpty(msg) && !eqauls(msg, "fail")){
				closepanel();
				window.location.href = url;
			}else{
				messageBox("抱歉,您输入的用户或密码不正确!");
			}
		}
	});	
};
...
$("#forgetPwd").click(function(){
	...
	$.ajax({
		url:getRootPath()+"/user/forget",
		type:"post",
		data:{"email":email},
		success:function(response){
			messageBox("恭喜您,密码找回成功!</br>请登录邮箱查看密码！")
		}
...
/* 通用自动居中(需要css定位)  */
function XYCenter(obj){
	var width = getClientWidth();
	var height = getClientHeight();
	var wp =  $(obj).width();
	var hp =  $(obj).height();
	$(obj).css({"top":(height - hp ) / 2 + "px","left":(width - wp) / 2 +"px"});
}
...
function messageBox(content){
	...
	/* 消息框的html */
	var html =  "<div id='messageMask'>"+
			    "	<div id='messageBox'>"+
				"		<div class='messageTitle'>"+
				"			<a href='javascript:void(0);' class='messageText' id='msgTitle'>"+title+"</a>"+
				"			<a href='javascript:void messageClose();' id='msgClose' title='关闭'>X</a>"+
				"		</div>	"+
				"		<div class='contentBox'>"+
				"			<div id='messageContext'>"+ content +"</div>"+
				"		</div>	"+
				"		<div class='buttonBox'>"+
				"			<a href='javascript:void messageClose();' class='messageButton'>"+btn+"</a>"+
	...
	XYCenter("#messageBox");
	/* 消息框定时关闭 */
	$("#messageMask").fadeIn('3000',function(){
		var s = 5;
		var timeText = $("#timeText");
		var text = timeText.text();
		myinterval = setInterval(function(){
			timeText.text(text +"( " + s + "秒后自动关闭 )");
			s = s-1;
			if(s===0){
...
function messageClose(){
	clearInterval(myinterval);
	$("#messageMask").fadeOut('3000',function(){
		$(this).remove();
	});
}
...
function confirmBox(content){
	...
	var html =  "<div id='messageMask'>"+
				...
				"		</div>	"+
				"		<div class='buttonBox'>"+
				"			<a href='javascript:void messageConfirm();' class='messageButton'>"+btn+"</a>"+				
				"			<a href='javascript:void messageClose();' class='messageButton'>关闭</a>"+
				"			<span id='timeText'></span>"+
				"		</div>"+
				"	</div>"+
				"	<div class='clear'></div>"+
